...
| Web 与浏览器 | 公网搜索、网页/远程 PDF 读取；受控浏览器打开、编号链接点击、后退、页内查找 | 浏览器无脚本、无 Cookie、无登录态、不提交表单；每跳重新做 SSRF/DNS 钉扎 |
...
| 工具规模治理 | 核心目录、按目标相关选择、`search_tool_catalog` 动态激活 | 完整工具 schema 不再无条件注入每一轮模型上下文 |

统一安全不变量：所有副作用工具都在注册表入口校验一次性 call-id 审批，并在执行前取得
`tool_invocations` 租约；调用方、恢复路径、Scheduler 或子 Agent 都不能绕过该入口。
