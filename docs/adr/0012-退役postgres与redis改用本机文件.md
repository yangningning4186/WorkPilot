...
1. **失去跨表事务。** 例如"版本激活与 `chunks.is_searchable` 投影同事务"（约束 10）在
   KB 这条路径上没有了：重建索引是**就地覆盖**，中途失败会让这个 KB 暂时没有索引。
   这是明知的取舍——重建是秒级，而"没有索引"是**显式失败**（`search_index` 直接拒绝并
   要求重建），不会安静地返回旧结果。真正不能接受的是安静，不是短暂不可用。
   `manifest.json` 仍然原子写，它是"这个库里有什么"的唯一事实来源。
2. **失去 embedding 空间的版本保护。** Postgres 靠 `document_versions` 的事务切换挡住
   "换了 embedding 模型但索引还是旧的"。文件系统上只能靠 `manifest.json` 里的
   **embedding 签名**（model + dimensions + revision），每次加载时比对，不一致就拒绝检索。
   换模型后旧向量和新查询向量不在同一个空间里，检索不会报错，只会安静地返回胡说八道——
   这个签名字段存在的全部理由就是把它变成显式失败。
...
- 约束 10（候选版本全部成功才事务激活）现在**只**约束还走版本表的路径；KB 这条路径由
  "签名不匹配即拒绝检索"承担同一个职责，写在 [04 §3](../04-知识与阅读设计.md)。
...
- `eval/cowork_runner.py` 仍要求 `COWORK_STORE_BACKEND=postgres`，**目前是坏的**，
  收编方式见 [06 §4.5](../06-评测体系.md)。
