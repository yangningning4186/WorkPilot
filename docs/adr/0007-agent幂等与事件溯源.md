...
2. **`retry_of` 字段自相矛盾**：既然允许重试，重试写新行时 `step_idx` 必然不同，
   唯一约束根本拦不住重复执行
3. **最要命的一条**：从 `interrupt` 或崩溃恢复时，系统只能确认最后一个已持久化
   checkpoint；其后尚未确认完成的执行片段可能重新执行，其中的副作用也会重放。
...
| **靠 checkpoint 天然去重** | 不写额外表 | checkpoint 恢复的是**状态**，不是**副作用**。执行片段重放时外部写入照样可能发生 |
