...
当前 suite 的 `origin=synthetic` 且 `review_status=pending_human_review`。它可用于工程回归，
但在 owner 完成人工复核前不能作为正式产品质量 baseline。
...
当前 `eval/suites/kb-rag-research-dev-v1.json` 有 26 条 synthetic 样本，状态仍是
`pending_human_review`；旧 `m1-dev-70` 快照属于已退役架构，不能充当当前文件系统 KB 的
baseline。
...
- config fingerprint、Git SHA、label；
...
会拒绝含 runner error、缺失 policy 必需指标、dataset/config fingerprint、Git SHA 或 run label
...
suite/policy/kind 对齐、ready baseline 的 schema/provenance/完整性，以及 replay bundle。`rebuild_required`
会显式报 warning 但 doctor 返回 `0`；目录或 ready 资源损坏返回 `2`。当前三条旧 baseline 都已
注册为 `rebuild_required`，不会被误判为发布可用。
...
backend/.venv/bin/python -m eval.regression snapshot \
  ...
  --output eval/snapshots/v2/cowork-core-50.json
```
...
backend/.venv/bin/python -m eval.regression check \
  eval/outputs/cowork-core/APPROVED_RUN/report.json \
  --baseline eval/snapshots/v2/cowork-core-50.json \
  --policy eval/policies/cowork.json
```
...
1. **冻结 suite**：owner 逐题复核 gold、可解性、split 和隐私；正式 baseline 的 suite 状态应
   是 human-reviewed；
...
当前 snapshot 命令不会替 owner 强制 `git_dirty=false`、human review 或双人批准；这些是运维
门禁，不是脚本已经自动实现的能力。
...
|---|---|---|---|
| `eval/snapshots/cowork.json` | 40 条 `cowork-core-40`，`items` 还是 `item_id -> bool`；当前 suite 是 50 条 v1.4.0，指标与 provenance 契约也已变化 | 完成 core-50 人工复核；在 clean Git、固定模型/预算下跑完整 dev；需要 test 时走冻结访问；生成新的 v1 baseline | 否；旧 40 条不能 rescore 成 50 条，也缺新 policy 所需指标 |
| `eval/snapshots/retrieval.json` | `m1-dev-70` 与已退役检索架构绑定；当前 runner 是文件系统 KB，当前候选 suite 是 26 条 `kb-rag-research-dev-v1` | 人工复核 26 条 stable anchors；固定 KB/index version 跑当前 runner；确认无 error 后生成新的 retrieval v1 baseline | 否；dataset/suite fingerprint 会按设计拒绝比较 |
