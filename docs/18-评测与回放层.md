...
4 条不可答，8 个证据文档与 evaluation gold 文档无交集。它仍是
`synthetic/pending_human_review`，必须先逐条复核。批准后先用与 evaluation 完全相同的
KB/index、Top-K、预算和 reranker 配置跑它（不传阈值），再冻结阈值；输出路径必须不存在：
...
|---|---|---|---|
| `eval/snapshots/cowork.json` | 40 条 `cowork-core-40`，`items` 还是 `item_id -> bool`；当前 suite 是 50 条 v1.4.0，指标与 provenance 契约也已变化 | 完成 core-50 人工复核；在 clean Git、固定模型/预算下分别跑 39 条 dev 与获授权的 11 条 test；生成 `v2/cowork-core-dev.json` 和 `v2/cowork-core-test.json` | 否；dev/test 不再共用 baseline 语义，旧 40 条也缺新 policy 所需指标 |
| `eval/snapshots/retrieval.json` | `m1-dev-70` 与已退役检索架构绑定；当前 runner 是文件系统 KB，当前候选 suite 是 26 条 `kb-rag-research-dev-v1` | 人工复核 26 条 stable anchors，尤其 4 条不可答题；另建并批准独立 calibration suite；固定 KB/index、真实 score source 和预算运行；生成新的 retrieval v1 baseline | 否；dataset/suite、calibration 和 score source 会按设计拒绝错误比较 |
