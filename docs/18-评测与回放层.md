...
当前 suite 是 `eval/suites/cowork-core-50-v1.6.json`，覆盖 workspace、knowledge、artifact、
...
当前 suite 保留生成来源 `origin=synthetic`。已批准的 v1.5.1 原文件继续冻结保留；v1.6.0
退役 `search_tool_catalog`、`load_skill_resource`、`list_workspace_roots` 并迁移四条 gold，当前
等待人工复核，不能沿用旧签字或 baseline。冻结 test 仍不得用于调参，dev/test 继续使用独立 baseline。
...
源报告必须属于同一个 `cowork-core-50` 版本且 suite SHA 完全一致；旧 `cowork-core-40` 或
v1.5.1 报告不能跨版本重评分后伪装成 v1.6.0。

```bash
PYTHONPATH=backend backend/.venv/bin/python -m eval.cowork_runner \
  --suite eval/suites/cowork-core-50-v1.6.json \
...
Cowork 评测的 token 维度固定为只计量、不熔断（manifest 中 `budgets.tokens=0`），避免用下一轮
最坏情况预留把尚未超过上限的正确任务判失败。调用次数与墙钟仍是独立安全门；旧命令里的
非零 `--budget-tokens` 会被明确拒绝，而不是静默改变评测口径。
