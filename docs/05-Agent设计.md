...
> Agent 状态是显式的、可 JSON 序列化的 dict，每次决策和工具执行都是 `state → state` 的确定性迁移。
...
```
  ...
  ★ 部署方显式配置预算时，每个节点执行前后都检查，超限 → budget_exceeded → END
```

（上图已作废，见 §2.2 开头。下面的计量口径仍然有效并且正在用。）

### 长任务计量与可选熔断

实现在 `app/agent_core/budget.py`。token、模型调用数和执行墙钟始终记账，但桌面 Cowork
默认把三个上限都设为 `0`（不限制）。长任务由用户取消、同签名重复调用刹车、上下文压缩、
每日费用上限和模型自然完成来终止，不再把“累计工作量大”当成任务失败。

无人值守或商用 API 部署仍可把任一上限设为正数。启用后，三个维度的判定口径如下：
...
4. **显式熔断不可重试。** worker 对普通失败有三次重试，但重跑同一节点只会继续烧同一份预算，
...
| 显式预算耗尽 | 仅部署方配置正数上限时进入 `budget_exceeded`；默认长任务不触发 |
| 用户中断 | `status=cancelled`，保留 checkpoint，支持后续恢复 |

### 5.1 幂等必须落在工具副作用边界 ★

**一个必须澄清的陷阱**：从 `interrupt` 或崩溃恢复时，系统只能确定最后一个已持久化
checkpoint；其后尚未确认完成的执行片段可能重新执行，其中的副作用也可能重放。
